...
def _resolve_html() -> Optional[Path]:
    """
    Find gui/components/template.html from common roots.
    Assumes you run uvicorn from repo root; falls back to relative to this file.
    """
    candidates = [
        Path("gui/components/template.html"),
        Path(__file__).resolve().parents[1] / "gui" / "components" / "template.html",
        Path(__file__).resolve().parents[2] / "gui" / "components" / "template.html",
    ]
    for p in candidates:
        if p.exists():
            return p
...
async def template_page(): # <-- Function modified
    html_path = _resolve_html()
    if not html_path:
        return PlainTextResponse(
            "template.html not found. Expected at gui/components/template.html",
            status_code=404,
        )
    html = html_path.read_text(encoding="utf-8")

    # Inject module/registry-provided assets for this page
    inject = _tags_for("/template")
    if inject:
        if "</body>" in html:
...
template_node = Node(
    ...
    meta={"entry_path": "/template", "icon": "🌱", "label": "Template"},
)