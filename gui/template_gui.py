...
def debug(*args, **kwargs):
    if DEBUG_ENABLED:
        print("[template_gui]", *args, **kwargs)

router = APIRouter(prefix="/api/project_template", tags=["Project Template"])
debug("Router initialized at /api/project_template")
...
from api.manifest.resolver import resolve_path, get_db_uri, RDS_ENABLED

# ---------- Internal helpers ----------

def _api_dir() -> Path:
    p = Path(__file__).resolve().parent
    debug("_api_dir:", p.as_posix())
    return p

def _repo_root() -> Path:
    r = _api_dir().parent
    debug("_repo_root:", r.as_posix())
    return r
...
def _projects_root() -> Path:
    ...
    debug("_projects_root:", root.as_posix())
    return root

def _project_path(project_name: str) -> Path:
    p = _projects_root() / project_name
    debug("_project_path:", project_name, "->", p.as_posix())
    return p
...
def _load_if_exists(path: Path) -> Any | None:
    ...
    try:
        ...
        debug("_load_if_exists: failed to load", path.as_posix(), repr(e))
        return None
...
def _validate_project_code(code: str) -> str:
    debug("_validate_project_code:", code)
    if not code or not _PROJECT_CODE_RE.match(code):
        raise HTTPException(
            status_code=400,
            detail="Invalid project_code. Use 2–64 characters: letters, numbers, dot, or hyphen (no underscores)."
        )
    return code
...
def _env_dsn() -> Optional[str]:
    ...
    debug("_env_dsn present?", bool(dsn))
    if dsn:
        return dsn
...
def _normalize_for_psycopg(url: str) -> str:
    """
    Convert SQLAlchemy-style or asyncpg-style URLs to psycopg-compatible.
    ...
    # psycopg expects sslmode instead of ssl
    if "?ssl=require" in url:
        url = url.replace("?ssl=require", "?sslmode=require")

    # double guard: if both asyncpg:// and postgresql:// appear (edge case)
...
def _effective_logins_dsn() -> Tuple[str, str]:
    ...
    dsn = _env_dsn()
    if dsn:
        debug("_effective_logins_dsn: using env DSN")
    ...
    try:
        ...
        debug("_effective_logins_dsn: resolver returned:", uri)
    except Exception as e:
...
def _open_db() -> _DBHandle:
    ...
    if kind == "postgres" and not _PSYCOPG_AVAILABLE:
        debug("_open_db: psycopg not available, falling back to SQLite")
...
def _exec(db: _DBHandle, sql: str, params: Tuple[Any, ...] = ()) -> None:
    debug("_exec:", {"kind": db.kind, "sql": sql[:80] + ("..." if len(sql) > 80 else ""), "params": params})
    if db.kind == "pg":
...
def _fetchone(db: _DBHandle, sql: str, params: Tuple[Any, ...] = ()) -> Optional[Tuple[Any, ...]]:
    debug("_fetchone:", {"kind": db.kind, "sql": sql[:80] + ("..." if len(sql) > 80 else ""), "params": params})
    if db.kind == "pg":
        with db.conn.cursor() as cur:
            cur.execute(sql, params)
            row = cur.fetchone()
...
def _ensure_projects_table(db: _DBHandle) -> None:
    debug("_ensure_projects_table:", db.kind)
    if db.kind == "pg":
        _exec(db, """
...
def _now_utc_iso() -> str:
    ...
    debug("_now_utc_iso:", ts)
    return ts

def _upsert_project_in_logins(project_name: str, project_code: str, description: Optional[str] = None) -> None:
    debug("_upsert_project_in_logins: begin", {"name": project_name, "code": project_code, "desc": description})
    with _open_db() as db:
        ...
        if row and row[0] and row[0] != project_code:
            raise HTTPException(
                status_code=409,
                detail=f"Project name '{project_name}' already exists with project_code '{row[0]}'."
            )

        if db.kind == "pg":
            debug("  upsert (pg) by PRIMARY KEY id=project_code")
            _exec(db, """
            ...
            debug("  upsert (sqlite) by project_code")
            _exec(db, """
                INSERT INTO projects (id, name, project_code, description, created_at)
                VALUES (%s, %s, %s, %s, %s)
    ...
    debug("_upsert_project_in_logins: done")
...
def export_project_template(project: str, download: bool = Query(True)):
    debug("GET /{project}/export:", {"project": project, "download": download})
...
def import_project_template(req: ImportTemplateRequest = Body(...)):
    ...
    debug("  target path:", target.as_posix())
    ...
    if not req.dry_run:
        debug("  performing DB upsert (not dry-run)")
    ...
    debug("POST /import: executing plan with", len(plan), "steps")
    for step in plan:
        op = step["op"]
        p = Path(step["path"])
        debug("  exec:", op, "->", p.as_posix())
        if op == "mkdir":
