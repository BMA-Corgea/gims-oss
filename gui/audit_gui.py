...
from core.core_audit import audit_all
...
debug("[audit_gui] Router initialized at /api/audit with tag 'Audit'")
...
def _resolve_project_path(project: str) -> Path:
    """
    Resolve a project path. Accepts absolute path or a project name living under repo_root/projects/<project>.
    """
    debug("[_resolve_project_path] input:", project)
    p = Path(project)
    if p.exists():
        debug("[_resolve_project_path] Using provided path:", p)
        return p.resolve()

    # Try repo_root/projects/<project>
    here = Path(__file__).resolve()
    repo_root = here.parent.parent if (here.name == "audit_gui.py") else here.parent
    candidate = repo_root / "projects" / project
    debug("[_resolve_project_path] candidate:", candidate)
    if candidate.exists():
        return candidate.resolve()

    raise HTTPException(status_code=404, detail=f"Project path not found: {project}")
...
def _load_all_schemas(project_path: Path) -> tuple[dict, dict, dict, dict]:
    ...
    """
    debug("[_load_all_schemas] Start for project:", project_path)
    try:
        ...
        debug("[_load_all_schemas] File not found:", e)
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        debug("[_load_all_schemas] Error:", e)
    ...
    for entry in (adj_list if isinstance(adj_list, list) else []):
        ...
        if key:
            if key in adjective_types:
                debug(f"[_load_all_schemas] ! duplicate adjective key in list: {key}")
    ...
    for entry in (adv_list if isinstance(adv_list, list) else []):
        ...
        if key:
            if key in adverb_types:
                debug(f"[_load_all_schemas] ! duplicate adverb key in list: {key}")
            adverb_types[key] = entry

    debug("[_load_all_schemas] Loaded:",
...
def _build_noun_instances_sql_only(project_path: Path, noun_types: dict) -> dict[str, list[dict]]:
    ...
    """
    debug("[_build_noun_instances_sql_only] Start")
    out: dict[str, list[dict]] = {}
    for noun_type in noun_types.keys():
        try:
            rows = get_noun_items(project_path, noun_type)  # SQL only in your i_o.py
            debug(f"[_build_noun_instances_sql_only] {noun_type}: {len(rows)} rows")
            out[noun_type] = rows
        except Exception as e:
            debug(f"[_build_noun_instances_sql_only] ! Failed for noun_type={noun_type}: {e}")
            out[noun_type] = []
    return out
        ...
        if pf:
            for inst in noun_instances_by_type.get(nt, []):
                if pf in inst and inst.get(pf):
                    ids.add(inst.get(pf))
        index[nt] = ids
        debug(f"[_build_noun_index] {nt}: pf={pf} count={len(ids)}")
    return index


def _list_verb_groups_safe(project_path: Path) -> list[str]:
    debug("[_list_verb_groups_safe] Start")
    try:
        groups = list_verb_groups(project_path)
        debug("[_list_verb_groups_safe] Found groups:", groups)
        return groups
    except Exception as e:
        debug("[_list_verb_groups_safe] ! list_verb_groups failed:", e)
        return []
...
def _load_runs_for_group(project_path: Path, group: str) -> tuple[list[dict], Optional[str]]:
    ...
    """
    debug(f"[_load_runs_for_group] group={group}")
    try:
        cfg = get_verb_group_log_config(project_path, group)
        primary_id = cfg.get("primary_id") or cfg.get("primaryId") or cfg.get("primary")
        debug(f"[_load_runs_for_group] primary_id field={primary_id}")
    except Exception as e:
        debug(f"[_load_runs_for_group] ! get_verb_group_log_config failed for {group}: {e}")
        primary_id = None

    try:
        entries = load_verb_group_log(project_path, group)
        debug(f"[_load_runs_for_group] entries={len(entries)}")
    except Exception as e:
        debug(f"[_load_runs_for_group] ! load_verb_group_log failed for {group}: {e}")
        entries = []
...
def _hydrate_run_entry(project_path: Path, group: str, run_id: str, base_entry: dict) -> dict:
    ...
    """
    debug(f"[_hydrate_run_entry] group={group} run_id={run_id}")
    ...
    try:
        ...
        debug(f"[_hydrate_run_entry] DataEntry.json -> {de_path} loaded={bool(run['data_entry'])}")
    except Exception as e:
        debug(f"[_hydrate_run_entry] ! data_entry load failed: {e}")
        run["data_entry"] = {}

    try:
        adv_path = resolve_path(project_path, "adverb_file", verb_group=group, run_id=run_id)
        run["adverbs"] = load_data(adv_path) or {}
        debug(f"[_hydrate_run_entry] adverbs.json -> {adv_path} loaded keys={list(run['adverbs'].keys()) if isinstance(run['adverbs'], dict) else 'n/a'}")
    except Exception as e:
        debug(f"[_hydrate_run_entry] ! adverbs load failed: {e}")
        run["adverbs"] = {}
    ...
    try:
        ...
        debug(f"[_hydrate_run_entry] Status.json -> {st_path} loaded={bool(run['status'])}")
    except Exception as e:
        debug(f"[_hydrate_run_entry] ! status load failed: {e}")
        run["status"] = {}
...
def _build_run_entries_by_group(project_path: Path, verb_types: dict) -> dict[str, list[dict]]:
    ...
    for group in groups:
        ...
        for i, row in enumerate(entries):
            ...
            debug(f"[_build_run_entries_by_group] row#{i} run_id={run_id} verb={verb_name}")

            # Hydrate data_entry + adverbs
            if run_id:
                hydrated.append(_hydrate_run_entry(project_path, group, run_id, {**row, **run_min}))
            else:
                debug(f"[_build_run_entries_by_group] ! missing run_id in group={group} row#{i}")
                hydrated.append({**row, **run_min, "data_entry": {}, "adverbs": {}})

        out[group] = hydrated
        debug(f"[_build_run_entries_by_group] group={group} hydrated={len(hydrated)}")
    return out
...
def _build_override_index(project_path: Path) -> dict[str, set]:
    ...
    """
    debug("[_build_override_index] Start")
    idx: dict[str, set] = {}
    try:
        entries = load_override(project_path) or []
    except Exception as e:
        debug("[_build_override_index] ! load_override failed:", e)
        entries = []
    ...
    for i, row in enumerate(entries):
        ...
        debug(f"[_build_override_index] row#{i} run={run} alias_of={alias_of}")

    debug("[_build_override_index] Built index keys:", list(idx.keys()))
    return idx
...
def audit_project(
    ...
    """
    debug("[audit_project] Start project=", project)
    project_path = _resolve_project_path(project)
    debug("[audit_project] Resolved project_path:", project_path)
    ...
    # 5) Core audit
    debug("[audit_project] Calling core audit_all()")
    result = audit_all(
    ...
    debug("[audit_project] audit_all() completed. Findings:", result["summary"]["total"])

    # 6) Attach debug extras if requested
    if include_noun_index:
        debug("[audit_project] include_noun_index=True")
        result["debug"] = {
...
def list_groups(project: str):
    debug("[list_groups] project=", project)
    project_path = _resolve_project_path(project)
    groups = _list_verb_groups_safe(project_path)
