...
def effective_policy(policy: Dict[str, Any], kind: str, name: str) -> Dict[str, Any]:
    ...
    kind: "nouns" | "verbs"
    """
    debug("[effective_policy] start kind=", kind, "name=", name)
    ...
    merged = dict(default)
    merged.update(block)
    debug("[effective_policy] merged ->", merged)
    return merged
...
def intersect_columns(
    ...
    """
    debug("[intersect_columns] hot_cols=", hot_cols, "arc_cols=", arc_cols)
    hot_names = [c[0] for c in hot_cols]
    if arc_cols is None:
        debug("[intersect_columns] archive columns unknown; using hot order")
        return hot_names
    arc_names = {c[0] for c in arc_cols}
    common = [c for c in hot_names if c in arc_names]
    debug("[intersect_columns] common ->", common)
    return common
...
def select_eligible_by_count(
    ...
    """
    debug("[select_eligible_by_count] total=", total_count, "max=", max_items)
    if total_count <= max_items:
        debug("[select_eligible_by_count] no overflow")
        return []
    surplus = total_count - max_items
    chosen = list(ordered_oldest_ids[:surplus])
    debug("[select_eligible_by_count] surplus=", surplus, "chosen=", chosen)
    return chosen
...
def select_eligible_by_age(
    ...
    *,
    ...
    """
    debug("[select_eligible_by_age] rows=", len(rows_for_age_eval), "id_field=", id_field, "date_field=", date_field, "days=", archive_after_days)
    ...
    for r in rows_for_age_eval:
        ...
        if parsed and parsed < cutoff:
            out.append(pid)
    debug("[select_eligible_by_age] chosen ->", out)
    return out
...
def plan_soft_archive_nouns(
    ...
    *,
    ...
    debug("[plan_soft_archive_nouns] noun=", noun_type, "table=", table, "pf=", primary_field, "ids=", list(primary_ids))
    ...
    if not ids:
        debug("[plan_soft_archive_nouns] no IDs; returning empty plan")
        return plan

    placeholders = ",".join(["?"] * len(ids))
    sql = f'UPDATE "{table}" SET archived=1, archived_at=? WHERE "{primary_field}" IN ({placeholders})'
    params = ( _now_iso(), *ids )
    debug("[plan_soft_archive_nouns] SQL:", sql, "params_len=", len(params))
    ...
    for pid in ids:
        ...
        debug("[plan_soft_archive_nouns] index insert for", pid)
...
def plan_hard_archive_nouns(
    ...
    *,
    ...
    debug("[plan_hard_archive_nouns] noun=", noun_type, "src_table=", src_table, "dst_table=", dst_table, "pf=", primary_field)
    plan = Plan(description=f"Hard archive {noun_type}", meta={"noun_type": noun_type, "strategy": "hard"})
    ids = list(primary_ids)
    debug("[plan_hard_archive_nouns] ids=", ids)
    ...
    common_cols = intersect_columns(hot_columns, archive_columns)
    ...
    if archive_columns:
        ...
        debug("[plan_hard_archive_nouns] Archive table exists, using common columns:", cols_to_use)
    else:
        # Archive table likely doesn't exist yet, use all hot columns
        cols_to_use = [c[0] for c in hot_columns]
        debug("[plan_hard_archive_nouns] Archive table likely new, using all hot columns:", cols_to_use)

    if not cols_to_use:
        debug("[plan_hard_archive_nouns] X No columns identified to copy; returning plan with only EnsureArchiveTable")
    ...
    for pid in ids:
        debug("[plan_hard_archive_nouns] steps for pid=", pid)
...
def plan_restore_nouns_soft(
    ...
    debug("[plan_restore_nouns_soft] noun=", noun_type, "table=", table, "pf=", primary_field, "ids=", list(primary_ids))
    plan = Plan(description=f"Soft-restore {noun_type}", meta={"noun_type": noun_type, "mode": "soft"})
    ids = list(primary_ids)
    if not ids:
        debug("[plan_restore_nouns_soft] no IDs; empty plan")
        return plan
    placeholders = ",".join(["?"] * len(ids))
    sql = f'UPDATE "{table}" SET archived=0, archived_at=NULL WHERE "{primary_field}" IN ({placeholders})'
    debug("[plan_restore_nouns_soft] SQL:", sql)
...
def plan_restore_nouns_hard(
    ...
    *,
    ...
    debug("[plan_restore_nouns_hard] noun=", noun_type, "src(archive)=", src_table, "dst(hot)=", dst_table, "pf=", primary_field)
    ...
    if not common:
        debug("[plan_restore_nouns_hard] X No common columns; returning empty plan")
        return plan
    ...
    for pid in ids:
        debug("[plan_restore_nouns_hard] steps for pid=", pid)
...
def plan_archive_runs_hard(
    ...
    *,
    ...
    """
    debug("[plan_archive_runs_hard] items=", len(items))
    ...
    for it in items:
        ...
        debug("[plan_archive_runs_hard] rid=", rid, "vg=", vg, "src=", src, "dst=", dst)
        # FS move
...
def plan_archive_runs_soft(
    ...
    *,
    ...
    and index it.
    """
    debug("[plan_archive_runs_soft] items=", len(items))
    plan = Plan(description="Soft archive runs", meta={"strategy": "soft"})
    for it in items:
        rid = it["run_id"]; verb = it["test_type"]; vg = it["verb_group"]
        src = it["src_dir"]
        marker = f"{src}/.archived"
        debug("[plan_archive_runs_soft] rid=", rid, "marker=", marker)
...
def plan_restore_runs(
    ...
    for it in items:
        ...
        if has_hard and arc_dir and hot_dir:
            ...
            debug("[plan_restore_runs] no-op for rid=", rid)
    return plan
...
def plan_apply_archive_policy_for_nouns(
    ...
    """
    debug("[plan_apply_archive_policy_for_nouns] start with", len(noun_tables), "noun types")
    ...
    for noun_type, info in noun_tables.items():
        ...
        if isinstance(max_items, int):
            debug("[policy] count-based max_items=", max_items)
        ...
        if isinstance(after_days, int):
            debug("[policy] age-based after_days=", after_days)
            ...
            if date_field:
                ...
                debug("[policy] no date_field available; skipping age rule")

        # Deduplicate preserve order
        seen = set()
        eligible = [x for x in eligible if not (x in seen or seen.add(x))]
        debug("[plan_apply_archive_policy_for_nouns] eligible ids ->", eligible)
        ...
        if strategy == "soft":
            ...
            debug("[plan_apply_archive_policy_for_nouns] unknown strategy:", strategy)
