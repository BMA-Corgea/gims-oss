...
def normalize_string(s: str) -> str:
    ...
    debug("[normalize_string] in:", repr(s), "-> out:", repr(result))
    return result
...
def _resolve_group_and_primary_field(item: dict, primary_id_by_group: dict) -> tuple[str | None, str | None]:
        ...
        if gf in item and isinstance(item[gf], str):
            for variant in _normalize_group_for_lookup(item[gf]):
                if variant in norm_map:
                    ...
                    debug("[_resolve_group_and_primary_field] via field", gf, "->", g_orig, "/", pid)
                    return (g_orig, pid)
    ...
    for g_lower, (g_orig, pid) in norm_map.items():
        ...
        if actual is not None:
            debug("[_resolve_group_and_primary_field] via presence of field", pid, "as", actual, "->", g_orig)
            return (g_orig, pid)

    debug("[_resolve_group_and_primary_field] unresolved")
    return (None, None)
...
def match_schema_definitions(search_term: str, schemas: dict) -> list[dict]:
    ...
    """
    debug("[match_schema_definitions] start", {"search_term": search_term})
    matches = []
    normalized_search = normalize_string(search_term)

    for part, schema_dict in schemas.items():
        if schema_dict is None:
            debug(f"[match_schema_definitions] skip '{part}': schema_dict is None")
            continue
        ...
        debug(f"[match_schema_definitions] part={part} items={total_items}")
        ...
        for key, value in items_iter:
            try:
                ...
                debug(f"[match_schema_definitions] warn: failed processing key='{key}' in part='{part}': {e}")

        debug(f"[match_schema_definitions] part='{part}' matched={local_count}")

    debug("[match_schema_definitions] done total_matches:", len(matches))
    return matches
...
def match_instances(
    ...
    for idx, item in enumerate(instances):
        ...
        if noun_type and primary_id_lookup and noun_type in primary_id_lookup:
            ...
            resolved_group = None
            debug(f"[match_instances][noun] idx={idx} noun_type={noun_type} primary_id={item_primary_id_field_from_cfg}")
        ...
        if (not item_primary_id_field_from_cfg) and primary_id_map_by_group:
            ...
            if "_primary_id_field_from_config" in item:
                ...
                debug(f"[match_instances][verb] idx={idx} using pre-tagged primary_id='{item_primary_id_field_from_cfg}' from group='{resolved_group}'")
            # 2) Trust the explicit _verb_group tag and look up its configured PID
                ...
                if find_actual_key(item, expected_pid_field):
                    ...
                    debug(f"[match_instances][verb] idx={idx} via trusted _verb_group='{verb_group}' primary_id='{expected_pid_field}'")
            ...
            if not item_primary_id_field_from_cfg:
                ...
                if pid:
                    ...
                    resolved_group = g
                    debug(f"[match_instances][verb] idx={idx} via fallback resolver group='{g}' primary_id='{pid}'")
        ...
        if item_primary_id_field_from_cfg:
            ...
            if actual_primary_key and actual_primary_key != item_primary_id_field_from_cfg:
                debug(f"[match_instances] idx={idx} primary id key resolved: {item_primary_id_field_from_cfg!r} -> {actual_primary_key!r}")
            elif not actual_primary_key:
                debug(f"[match_instances] idx={idx} primary id key {item_primary_id_field_from_cfg!r} not present (any-casing/spacing)")
        ...
        for field in potential_id_fields:
            ...
            if search_term.lower() == value.lower():
                ...
                debug(f"[match_instances] idx={idx} HIGH exact_id field={field} value={value}")
                break
...
def deduplicate_matches(matches: list, primary_id_lookup: dict) -> list:
    ...
    """
    debug("[deduplicate_matches] start", {"incoming": len(matches)})
    seen = set()
    unique = []
    deduped = 0
    ...
    debug("[deduplicate_matches] done", {"outgoing": len(unique), "deduped": deduped})
    return unique
...
def cascade_deep_search(
    ...
    """
    debug("[cascade_deep_search] start", {
        "search_term": search_term,
    ...
    debug("[cascade_deep_search] schema_matches:", len(schema_matches))
    ...
    debug("[cascade_deep_search] noun_matches:", len(noun_matches))
    ...
    debug("[cascade_deep_search] verb_matches:", len(verb_matches))
    ...
    debug("[cascade_deep_search] unique_matches:", len(unique_matches))
    ...
    debug("[cascade_deep_search] done", {
