...
try:
    ...
    if DEBUG_ENABLED:
        print("[core_noun] psycopg not available:", repr(e))
...
def _pytype_to_sqlite(ftype: str) -> str:
    if ftype in ("string", "date", "adjective"):
        return "TEXT"
    if ftype in ("float", "number"):
        return "REAL"
    return "TEXT"
...
def _pytype_to_pg(ftype: str) -> str:
    if ftype in ("string", "adjective", "date"):
        return "TEXT"
    if ftype in ("float", "number"):
...
class NounType:
    ...
    def __init__(self, name: str, schema: dict, project_path: Path):
        ...
        debug(f"Initialized NounType(name={name})")
        try:
            self._sync_sql_table_non_destructive()
        except Exception as e:
            debug(f"Table ensure skipped (init): {e}")
