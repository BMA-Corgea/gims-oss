...
from api.routers.verb._db import _get_objects_db_target, _table_name, _ensure_verb_table
...
def _group_name(pp: Path, verb: str) -> str:
    vt = _verb_types(pp)
    if verb not in vt:
        raise AppError("VERB_NOT_FOUND", f"Verb '{verb}' not found.", status=404, details={"verb": verb})
    g = vt[verb].get("verb_group")
    if not g:
        raise AppError("VERB_MISSING_GROUP", f"Verb '{verb}' missing verb_group.", status=400, details={"verb": verb})
    return g
...
def _write_initial_status(pp: Path, group: str, verb: str, run_id: str):
    ...
    if ls_block:
        ...
        log.debug("[_write_initial_status] linear_status initialized", {"run_id": run_id, "steps": len(ls_block.get("steps", []))})
    else:
        doc = {"linear_status": {"enabled": False, "steps": []}}
        log.debug("[_write_initial_status] linear_status disabled for verb", verb)

    status_path.write_text(json.dumps(doc, indent=2))
    log.debug("[_write_initial_status] wrote Status.json", str(status_path))
...
def _ensure_dump_scaffold(pp: Path, group: str, verb: str, run_id: str):
    ...
    log.debug("[_ensure_dump_scaffold] dump_root:", str(dump_root))
...
def list_runs(project: str, verb: str):
    ...
    for r in rows:
        ...
        out.append(data)
    log.debug("[list_runs]", {"verb": verb, "group": group, "rows": len(out)})
    return out
...
def get_run(project: str, verb: str, run_id: str):
    ...
    if not row:
        log.debug("[get_run] not found", {"pid": pid_field, "run_id": run_id})
        raise AppError("RUN_NOT_FOUND", f"Run '{run_id}' not found.", status=404, details={"verb": verb, "run_id": run_id})

    data = dict(row["data"] or {})
    data["test_type"] = verb
    data[pid_field] = row["primary_id"]
    log.debug("[get_run] found", {"pid": pid_field, "run_id": run_id})
    return data
...
def validate_payload(project: str, verb: str, payload: Dict[str, Any] = Body(...)):
    ...
    ok = len(errors) == 0
    log.debug("[validate_payload]", {"ok": ok, "errors": errors})
...
def create_run(project: str, verb: str, payload: Dict[str, Any] = Body(...)):
    ...
    if pid_field not in payload or not str(payload.get(pid_field, "")).strip():
        raise AppError("MISSING_PRIMARY_ID", f"Payload must include primary id field '{pid_field}'.", status=400, details={"primary_id_field": pid_field})

    v = validate_payload(project, verb, payload)
    if not v["ok"]:
        log.debug("[create_run] validation failed", v["errors"])
        return v
    ...
    if _select_one_run(pp, verb, primary_id_value) is not None:
        log.debug("[create_run] duplicate", {"pid": pid_field, "val": primary_id_value})
        raise AppError("DUPLICATE_PRIMARY_ID", f"Duplicate primary id '{primary_id_value}' for verb '{verb}'.", status=409, details={"verb": verb, "primary_id": primary_id_value})

    # insert
    _insert_run(pp, verb, group, primary_id_value, payload)
    log.debug("[create_run] inserted", {"pid": pid_field, "val": primary_id_value})
...
def update_run(project: str, verb: str, run_id: str, payload: Dict[str, Any] = Body(...)):
    ...
    if not existing:
        log.debug("[update_run] not found", {"pid": pid_field, "run_id": run_id})
        raise AppError("RUN_NOT_FOUND", f"Run '{run_id}' not found.", status=404, details={"verb": verb, "run_id": run_id})
    ...
    if rc <= 0:
        raise AppError("UPDATE_FAILED", "Update failed unexpectedly.", status=500, details={"verb": verb, "run_id": run_id})

    log.debug("[update_run] replaced in SQL", {"pid": pid_field, "id": run_id})
...
def refresh_status_for_run(
    ...
    if not actual_verb:
        log.debug("[refresh_status_for_run] not found in SQL", {"run_id": run_id})
        raise AppError("RUN_NOT_FOUND", f"Run '{run_id}' not found in SQL.", status=404, details={"run_id": run_id})

    if actual_verb != verb:
        log.debug("[refresh_status_for_run] verb param differs from resolved", {"param": verb, "actual": actual_verb})

    vt = _verb_types(pp)
    if actual_verb not in vt:
        raise AppError("VERB_SCHEMA_NOT_FOUND", f"Verb schema for '{actual_verb}' not found.", status=404, details={"verb": actual_verb})

    group = vt[actual_verb].get("verb_group")
    if not group:
        raise AppError("VERB_MISSING_GROUP", f"Verb '{actual_verb}' missing verb_group.", status=400, details={"verb": actual_verb})

    # (Optional) ensure the run exists for that verb (it should, as we just resolved it)
    row = _select_one_run(pp, actual_verb, run_id)
    if not row:
        log.debug("[refresh_status_for_run] unexpected: row missing on resolved verb", {"actual": actual_verb, "run_id": run_id})
    ...
    try:
        ...
        log.debug("[refresh_status_for_run] could not re-read Status.json for step count", {"run_id": run_id, "status_path": str(status_path)}, exc_info=True)
        doc, steps = {}, 0
