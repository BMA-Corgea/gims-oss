...
def format_table(project: str, noun_type: str, payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    if not isinstance(records, list):
        raise AppError("INVALID_RECORDS", "Expected list of records", status=400)

    proj_root = resolve_path(Path(), "project_root") / project
    noun_schema = get_noun_schema(proj_root, noun_type)
    if not noun_schema:
        raise AppError("NOUN_SCHEMA_NOT_FOUND", f"Missing noun schema for {noun_type}", status=404, details={"noun_type": noun_type})
...
def lineage_ui(project: str, noun_type: str, payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    if not isinstance(record, dict):
        raise AppError("INVALID_RECORD", "Missing or invalid 'record'", status=400)
    ...
    for n in all_noun_schemas.keys():
        try:
            ...
            log.debug("Preload of noun items failed for %r (project=%r); skipping", n, project, exc_info=True)
            pass
    ...
    try:
        ...
        log.warning("Failed to list verb groups (project=%r); treating as empty", project, exc_info=True)
        groups = []
    ...
    for g in groups:
        try:
            ...
            log.warning("Failed to load verb group log config (project=%r, group=%r)", project, g, exc_info=True)
            pid = ""

        try:
            entries = load_verb_group_log(proj_root, g)
        except Exception:
            log.warning("Failed to load verb group log (project=%r, group=%r)", project, g, exc_info=True)
            entries = []
    ...
    for run in lineage.get("runs", []):
        ...
        if not verb_name:
            try:
                ...
                log.debug("Failed to resolve verb name for run_id=%r (project=%r)", run_id, project, exc_info=True)
                verb_name = ""

        try:
            verb_group = resolve_verb_group_from_test_type(proj_root, verb_name) or "Tests"
        except Exception:
            log.debug("Failed to resolve verb group for verb=%r (project=%r); defaulting to 'Tests'", verb_name, project, exc_info=True)
            verb_group = "Tests"
